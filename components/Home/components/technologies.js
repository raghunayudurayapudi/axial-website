...
const Technologies = () => {
  return (
    <section id="technologies" >
        <div className={`${css.aboutHeader} mt-[160px]`}>Technologies</div>
        ...
        <div className='mt-[73px]'>
            {content.map((value, index) => (
                ...
                ${index === 4 ? "rounded-bl-md rounded-br-md" : ""} ${index !== 0 ? "border-t-0" : ""} `}>
                    <div className={`text-[36px] text-[#032246] font-[700] `}>{value.header}</div>
                    <div className={`mt-[25px] text-[18px] text-[#032246] max-w-[328px] md:max-w-[375px] ${index % 2 === 1 ? "md:text-[#84584D]": ""}`}>{value.content}</div>
                    <div className={`flex items-center gap-[13px] mt-[30px] text-[#141211] md:hidden`}>READ MORE <ReadMoreIcon /></div>
